...
```bash
sudo adduser alumne3
sudo usermod -aG sudo alumne3
groups alumne3
id alumne3
```
...
**Què fa**  
Crea un usuari nou anomenat `alumne3`.
...
Per exemple, si crees `alumne3`, el més habitual és que es creï també:
- usuari: `alumne3`
- grup principal: `alumne3`
...
**Què fa**  
Afegeix l’usuari `alumne3` al grup `sudo`, és a dir, li dona permisos administratius.
...
**Què fa**  
Mostra els grups als quals pertany l’usuari `alumne3`.
...
```text
uid=1001(alumne3) gid=1001(alumne3) groups=1001(alumne3),27(sudo)
```

Això indica que:
- l’usuari és `alumne3`
- el grup principal també és `alumne3`
...
```bash
sudo chown alumne3:alumne3 practica/docs/notes.txt
chmod 640 practica/docs/notes.txt
sudo passwd alumne3
sudo usermod -L alumne3
sudo usermod -U alumne3
```
...
**Què significa `alumne3:alumne3`**
- primer `alumne3` = nou propietari
- segon `alumne3` = nou grup
...
**Què fa**  
Canvia la contrasenya de l’usuari `alumne3`.
